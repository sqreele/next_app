# ==============================================================================
# File: backend/my_app/admin.py (COMPLETE FIX for Foreign Key Issues)
# Description: SQLAdmin configuration with working foreign key dropdowns
# ==============================================================================
...
# *** COMPLETELY FIXED PROCEDURE ADMIN ***
class ProcedureAdmin(ModelView, model=Procedure):
    ...
    # Override form creation to add custom machine selection
    def scaffold_form(self):
        form_class = super().scaffold_form()
        
        # Replace the default machine_id field with a custom SelectField
    ...
    def create_form(self, obj=None):
        ...
        # Populate machine choices dynamically
        from .database import sync_engine
        from sqlalchemy.orm import sessionmaker
        
        # Create a synchronous session for SQLAdmin
        Session = sessionmaker(bind=sync_engine)
        with Session() as session:
            machines = session.execute(select(Machine)).scalars().all()
            machine_choices = [(m.id, f"{m.name} (ID: {m.id})") for m in machines]
            
            # Add a default "Select a machine" option
            form.machine_id.choices = [('', 'Select a machine...')] + machine_choices
    ...
    def edit_form(self, obj=None):
        ...
        # Populate machine choices for edit form too
        from .database import sync_engine
        from sqlalchemy.orm import sessionmaker
        ...
        with Session() as session:
            machines = session.execute(select(Machine)).scalars().all()
            machine_choices = [(m.id, f"{m.name} (ID: {m.id})") for m in machines]
            form.machine_id.choices = machine_choices
        
        return form
    
    # Add validation before saving
    async def insert_model(self, request, data):
        if not data.get('machine_id') or data.get('machine_id') == '':
            raise ValueError("Machine selection is required")
