...
async def register(
    ...
    try:
        ...
        existing_user = await crud.user.get_by_username(db, username=user_data.username)
        if existing_user:
            raise HTTPException(
        ...
        existing_email = await crud.user.get_by_email(db, email=user_data.email)
        if existing_email:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        # Note: You'll need to add password field to User model or handle differently
...
async def refresh_token(
    ...
    """Refresh access token - allows slightly expired tokens"""
    try:
        ...
        # Check if token is not too old (allow refresh within 5 minutes of expiration)
        exp = payload.get("exp")
        if exp:
            now = datetime.utcnow().timestamp()
            if now - exp > 300:  # 5 minutes grace period
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
